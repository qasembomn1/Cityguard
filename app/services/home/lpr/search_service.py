...
class LprSearchService:
    ...
    def _request_with_fallback(
        ...
        if last_exc is not None:
            raise last_exc
        raise RuntimeError("No LPR search API attempts configured.")
    ...
    def _extract_items(self, payload: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(payload, dict):
            for key in ("data", "result", "payload"):
                ...
                if nested_items:
                    return nested_items
            if "id" in payload and any(key in payload for key in ("number", "plate_no", "plate")):
                return [payload]
        elif isinstance(payload, list):
            return [item for item in payload if isinstance(item, dict)]
        return []

    def search_lpr(self, payload: Dict[str, Any] | LprSearchPayload) -> List[LprSearchResult]:
        data = payload.to_dict() if isinstance(payload, LprSearchPayload) else dict(payload or {})
        response = self._request_with_fallback(
            (
                ("POST", "/api/v1/search/lpr"),
                ("POST", "/api/v1/search/lpr/"),
            ),
            data=data,
        )
        return [
            LprSearchResult.from_dict(item)
